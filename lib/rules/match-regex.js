...
module.exports = function(context) {
    var defaultRegexp = /^([a-z0-9]+)([A-Z][a-z0-9]+)*$/g,
        ...
        ignoreExportingClass = context.options[2] ? context.options[2] : false;
    ...
    return {
        "Program": function(node) {
            var filename = context.getFilename(),
                ...
                isExportingClass = Boolean(getExportedClass(node, context)),
                matchesRegex = conventionRegexp.test(parsed.name);

            if (shouldIgnore) return;
            if (ignoreExporting && isExporting) return;
            if (ignoreExportingClass && isExportingClass) return;
            if (!matchesRegex) {
